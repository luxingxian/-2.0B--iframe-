...
function upOrNextMonth(){//防止函数找不到，有的页面不存在该函数,有的页面存在该函数，用于页面刷新后时间回显
}

function cleanTopTi(){//防止函数找不到，有的页面不存在该函数,有的页面存在该函数，用于页面刷新后时间回显
}
...
Date.prototype.format2 = function() {  
  ...
  function getAll(begin, end, top) {  
      ...
      for (var k = unixDb; k <= unixDe;) {  
    	  var time = (new Date(parseInt(k))).format2();  
          if(begin != time && end != time){
         	  var ids = new Date(time).Format("yyyy-M-d");
	          ids = top+"_"+ids;
	          $("#"+ids).addClass("ta_dateRangeSelected");
          } 
          k = k + 24 * 60 * 60 * 1000;  
      }  
  } 
