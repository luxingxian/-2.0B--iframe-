...
(function(root, factory) {
	...
	jedfn.init = function() {
		...
		createDiv.css({
			"z-index": zIndex,
			"position": (jet.fixed == true ? "absolute" : "fixed"),
			"display": "block"
		});
	...
	jedfn.chooseDays = function(opts) {
		var that = this,
			elemCell = that.valCell,
			boxCell = $(jet.boxCell);
		if("select_jate"==elemCell.attr("id")&&"changeFlag"==elemCell.attr("Val")){
				elemCell=$("#select_jate");
			}
		boxCell.find(".jedaul li").on("click", function(ev) {
			if($(this).find("i.marks").is(":visible")) {
				return;
			}
